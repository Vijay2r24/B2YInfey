    ...
    <div className={`relative w-full h-full flex flex-col justify-center bg-white overflow-hidden ${isActive ? 'section-active' : ''}`}
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-10 w-full">
            ...
            {[...clientLogos, ...clientLogos].map((client, i) => (
              <div key={i} className="flex-[0_0_25%] md:flex-[0_0_25%] min-w-0 px-2.5">
                <div className="bg-white rounded-[20px] p-4 flex items-center justify-center shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1"
                  style={{ borderTop: '4px solid #00bcd4', minHeight: '90px' }}>
        ...
        <div className="section-content delay-2 text-right mb-6">
          <a href="#clients" className="text-[#222] text-sm hover:text-[#00bcd4] transition-colors duration-300">...More</a>
        </div>
            ...
            {testimonials.map((item) => (
              <div key={item.id} className="flex-[0_0_100%] min-w-0 px-4">
                <div className="max-w-2xl mx-auto text-center py-4">
                  <svg className="w-8 h-8 text-[#00bcd4]/30 mx-auto mb-4" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  <p className="text-[#00bcd4] text-sm font-medium">— {item.author}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="section-content delay-4 flex items-center justify-center gap-4 mt-3">
          <button onClick={testPrev} className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-gray-400 hover:border-[#00bcd4] hover:text-[#00bcd4] transition-all duration-300 hover:scale-110">
            <ChevronLeft size={16} />
          </button>
          <div className="flex gap-2">
            {testimonials.map((_, i) => (
              <button key={i} onClick={() => testApi && testApi.scrollTo(i)}
                className={`h-2 rounded-full transition-all duration-500 ${i === testIndex ? 'bg-[#00bcd4] w-8' : 'bg-gray-300 w-2'}`} />
            ))}
          </div>
          <button onClick={testNext} className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-gray-400 hover:border-[#00bcd4] hover:text-[#00bcd4] transition-all duration-300 hover:scale-110">
